...
export const fetchCorrespondents = async (
  blankFirst?: boolean
): Promise<correspondent[]> => {
  try {
    const response = await fetch(`${paperlessURL}/api/correspondents/`, {
    ...
    await showToast(
      Toast.Style.Failure,
      `Could not fetch correspondents ${error}`
    );
    return Promise.reject([]);
  }
};

export const cacheCorrespondents = async (
  value: correspondent[]
): Promise<correspondent[]> => {
